...
water <- st_read("D:/example_data/water/water.shp")
...
#--- test on single road ---#
roadx <- roads %>%
  dplyr::filter(FID %in% c(22))

```

```{r, echo = FALSE}
map2 <- mapview::mapview(list(ctg, roadx), layer.name = c("catalog","road"),
                 color = "red", map.type = "Esri.WorldImagery")

coords <- c(-80.45357,
            46.97875)

map2@map %>% setView(coords[1], coords[2], zoom = 17)
...
corrected_road <- MFFProads::measure_road(road = roadx, ctg = ctg, dtm = dtm, water = water, param = param)

mapview::mapview(list(roadx, corrected_road),
